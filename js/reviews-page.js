...
} from "./check-penalties.js";

// Feature flag: desative as gotas para evitar pontos visíveis
const COFFEE_DROPLETS_ENABLED = false;
...
function createCoffeeDroplets(element, isAdding = true) {
  // Early exit when droplets are globally disabled
  if (!COFFEE_DROPLETS_ENABLED) return;

  const rect = element.getBoundingClientRect();
  const centerX = rect.left + rect.width / 2;
  const centerY = rect.top + rect.height / 2;

  // Cleanup: remove any old droplets older than 2.5s to avoid accumulation
  try {
    const now = Date.now();
    document.querySelectorAll(".droplet").forEach((d) => {
      const created = Number(d.dataset.created) || 0;
      if (created && now - created > 2500) d.remove();
    });
  } catch (e) {
    /* ignore */
  }
    ...
    dropletsConfig.forEach((config) => {
      for (let i = 0; i < config.count; i++) {
        setTimeout(() => {
          ...
          droplet.dataset.created = String(Date.now());
          document.body.appendChild(droplet);
          droplet.addEventListener("animationend", () => droplet.remove(), {
            once: true,
          });
          // Fallback: garantir remoção caso animationend não dispare
          setTimeout(() => {
            if (droplet.parentElement) droplet.remove();
          }, 1400);
        }, config.delay);
      }
    });
...
async function loadBooks() {
  ...
  try {
    ...
    console.warn(
      "Falha ao importar vejamais.js, usando fallback parcial.",
      err
    );
...
async function renderUserReviews() {
  ...
  const cardsHTML = reviews
    .map((review) => {
      const bookInfo =
        bookDataCache[review.bookId] || bookDataFallback[review.bookId];
      const tituloLivro = bookInfo
        ? bookInfo.titulo
        : `Livro (ID: ${review.bookId})`;
      ...
      return `
<article class="book-item review-card" data-id="${esc(
        review.bookId
      )}" data-title="${esc(tituloLivro)}">
    <div class="book-card"><img src="${esc(imgLivro)}" alt="${esc(
        tituloLivro
      )}" onerror="this.src='/imagens/placeholder.svg'"></div>
    <div class="book-title">
        ...
        <div class="botao-quero-ler">
            <button class="action-btn btn-remove" data-book-id="${esc(
              review.bookId
            )}" data-review-timestamp="${review.timestamp}">Deletar</button>
        </div>
    </div>
    <div class="book-action"><a href="vejamais.html?id=${encodeURIComponent(
      review.bookId
    )}&review=${encodeURIComponent(
        review.timestamp
      )}" class="review-link">Leia mais</a></div>
</article>`;
    })
    .join("");
...
async function createFloatingReviewCard() {
  ...
  const options = Object.keys(bookDataCache)
    .map((id) => `<option value="${id}">${bookDataCache[id].titulo}</option>`)
    .join("");
  ...
  card.innerHTML = `
        ...
        <label for="fr-rating-stars">Avaliação</label>
        <div id="fr-rating-stars" role="radiogroup" aria-label="Avaliação" ></div>
  ...
  toggleBtn.addEventListener("click", () => {
    if (!auth.currentUser) {
      if (
        confirm(
          "Você precisa estar logado para adicionar uma review. Ir para login?"
        )
      ) {
  ...
  // Inicializa widget de estrelas com suporte a meio-valor (0.5)
  function initHalfStarRating(container) {
    const count = 5;
    let current = 5; // default

    // Cria elementos de estrela
    for (let i = 1; i <= count; i++) {
      const sp = document.createElement("span");
      sp.className = "star";
      sp.dataset.value = i;
      sp.setAttribute("role", "radio");
      sp.setAttribute("aria-label", i + " estrelas");
      sp.innerText = "★";
    ...
    // Detecta posição do mouse para decidir meia-estrela
    function pointerValue(evt, el) {
      const rect = el.getBoundingClientRect();
      const x = evt.clientX - rect.left;
      return x < rect.width / 2
        ? Number(el.dataset.value) - 0.5
        : Number(el.dataset.value);
    }

    // Hover preview
    container.addEventListener("mousemove", (ev) => {
      const el = ev.target.closest(".star");
      if (!el) return;
      const v = pointerValue(ev, el);
      render(v);
    });
    ...
    // API mínima para obter rating
    container.getRating = () => current;
    // inicial render
    render(current);
    return container;
  }

  const frStars = initHalfStarRating(card.querySelector("#fr-rating-stars"));
  ...
  card.querySelector(".btn-submit").addEventListener("click", async () => {
    ...
    const rating = Number(frStars.getRating());
    ...
    try {
      ...
      const newCard = reviewsSection.querySelector(
        `[data-id="${CSS.escape(bookId)}"]`
      );
      if (newCard) {
