...
async function renderBooksList() {
  ...
  bookIds.forEach((id) => {
    ...
    const badge = isAdminBook
      ? '<span class="badge-custom">Adicionado pelo Admin</span>'
...
function updateGenreSelect() {
  ...
  const customGenres = JSON.parse(
    localStorage.getItem("custom-genres") || "[]"
  );
...
document
  .querySelectorAll(".modal .modal-close, .modal .btn-cancel")
  .forEach((btn) => {
...
window.deleteBook = function (id) {
  ...
  if (!adminBooks[id]) {
    alert(
      "Apenas livros adicionados pelo admin podem ser deletados.\n\nLivros originais do vejamais.js são permanentes."
    );
    return;
  }
...
function cleanBookFromUserLists(bookId) {
  ...
  const listKeys = ["quer-ler", "ja-lidos", "favoritos"];
...
document
  .getElementById("book-genero-select")
...
form?.addEventListener("submit", async (e) => {
  ...
  const descricaoCurta = descricaoCurtaInput
    ? descricaoCurtaInput.value.trim()
    : "";
  ...
  if (!fixedGenres.includes(genero)) {
    ...
    if (!exists) {
      ...
      const beforeOpt = Array.from(generoSelect.options).find(
        (o) => o.value === "novo"
      );
  ...
  // Garantir que a classe que trava o scroll/estado de modal seja removida
  document.body.classList.remove("modal-open");
...
async function renderReviewsList() {
  ...
  reviews.forEach((review) => {
    ...
    const userLabel =
      review.username && review.username !== "Usuário Anônimo"
        ? review.username
    ...
    card.innerHTML = `
            <div class="review-admin-info">
                ...
                <div class="review-admin-text">${
                  review.text || "<em>Sem comentário</em>"
                }</div>
            </div>
            <button class="btn-danger" onclick="window.deleteReviewAdmin('${
              review.bookId
            }', '${review.timestamp}')">Deletar</button>
        `;
...
function loadCustomGenres() {
  ...
  customGenres.forEach((genre) => {
    ...
    if (!exists) {
      ...
      newOption.value = genre;
      newOption.textContent = genre;
      const last = Array.from(generoSelect.options).find(
        (o) => o.value === "novo"
      );
